...
/**
 * The available settings for a Turbo implementation including the ability to
 * enable and disable specific types of analytics tracking. Many of these
 * settings configure a corresponding set of Turbo {@link Action | actions}.
 *
 * @remarks
...
/**
 * The default {@link ITurboSettings | settings} for a Turbo implementation,
...
/**
 * The events used by the {@link Turbo} interface to capture analytics data from
 * a world.
 *
 * @remarks
 * Most Turbo events have a corresponding Turbo {@link Action | action} that is the
 * context for the event. For example, the context of the `OnAbilityEquip` event
 * is the `ABILITY_EQUIP` action that is triggered when a player equips an
 * ability.
 *
 * Before you use Turbo events and actions to capture analytics data, you
 * must ensure the corresponding settings are enabled in the {@link ITurboSettings}
 * interface.
 *
 * To capture a Turbo event, you can pass the event and event payload to the
 * `Turbo.send` method. Event payloads are listed in the Type Alias section of
 * the `analytics` package topic. For example, the payload for the `OnAbilityDequip`
 * event, is the {@link OnAbilityDequip} type.
 *
 * @example
 * This script captures the `OnAreaEnter` event, which is triggered by the
 * `AREA_ENTER` action when the player enters an area. The payload type for
 * the event is `AreaEnterPayload`.
 * ```
...
/**
 * The main interface for managing In-World Analytics events in Horizon Worlds,
 * including registering the API for use in your world, and capturing analytics
 * events.
 *
 * @remarks
 * The `Turbo` variable is an interface used to create a custom analytics
 * manager in TypeScript that captures Turbo Analytics events.
 *
 ...
 *
 * 3. Optionally, if you aren't using the {@link TurboDefaultSettings | default}
 * Turbo settings, configure the ananlytics categories you want to capture in
 ...
 *
 * 1. Add an {@link core#Entity | entity} to your world that you can attach to the
 * the Analytics API in your script. This can be an empty entity.
 *
 * 4. Create an `AnalyticsManger` script and attach it the entity.
 *
 * 5. Register the entity with the `Turbo` interface by calling `Turbo.register`
 * and then pass the script and `turboSettings` to the `register` method. For
 * information about using the register method, see
 * {@link core#Component.register}.
 *
 ...
 *
 * @example
 * This example demonstrates how to register a world with the Analytics API. In
 * the example, an `AnalyticsManager` script is attached to an entity
 * in the world, and then the entity is registered with the Turbo interface.
 * ```
 ...
 *
 * @example
 * To capture Analytics events using the `Turbo` interface, call the
 * `Turbo.send` method. Here's an example.
 * ```
 * Turbo.send(TurboEvents.OnAreaEnter, player, payload);
 * ```
 */
